...
X 平台就是很典型的例子。你给普通 AI Agent 一个单条推文链接，它经常不能直接读到推文本体。直接抓 `x.com/.../status/...`，拿到的可能是登录页、JS shell、空页面，或者一堆登录、注册、推荐内容。退一步用公开搜索结果查，通常也只能看到一点片段、标题缓存、第三方转述，不是完整原文，也不一定是最新内容。

这在金融和研究场景里很危险。搜索片段不是原始来源，推荐栏不是正文，登录提示不是证据，URL 里的数字也不能当成金融数据。`pyaireader` 要解决的就是这件事：尽量读到原文；读不到时，也要明确告诉 Agent 读取质量、失败原因和 trace，而不是拿搜索片段假装读完。
